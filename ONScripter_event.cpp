...
SDL_PushEvent(&ev2);
    ...
    event.key.type = SDL_KEYDOWN;
    event.key.keysym.sym = transJoystickButton(event.cbutton.button);
    if (event.key.keysym.sym != SDLK_UNKNOWN) {
        event.key.keysym.sym = transKey(event.key.keysym.sym);
        ret = keyDownEvent( &event.key );
        if ( btndown_flag )
